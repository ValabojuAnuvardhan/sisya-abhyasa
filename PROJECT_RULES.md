...
> **Single Source of Truth** for the Get Shit Done methodology & Antigravity Operating Core.
> 
...
## GitHub Evidence Workspace — Architecture Lock v1.0

### 1. Core Principle (Locked)
**Freeze the layout, not the contents.**
The visual structure of the GitHub Evidence Workspace is frozen. Future engineering sprints (Sprints 3–7) enrich existing component containers instead of moving, removing, or redesigning them.

### 2. Permanent Component Hierarchy
```text
GitHubEvidencePage
│
├── GitHubHeader (Permanent Module Title)
├── GitHubConnectionCard (OAuth Status & Actions ONLY)
├── LinkedRepositoryBanner (Repository Link & Synchronization Control Center ONLY)
├── EvidenceTabs (Extensible Navigation: Timeline, PRs, Commits, Branches, Reviews, Issues, Releases, AI Verification)
├── MainContent (Canonical Evidence Stream Views)
├── RecentActivityFeed (Project Activity & Event Notification Stream)
└── RightSidebar
    ├── RepositorySummarySidebar (Repository Metrics ONLY)
    ├── SkillsEarned (Evidence-Backed Skill Bars)
    ├── RepositoryHealthCard (Sync & Rate Limit Health)
    ├── EvidenceScoreCard (Sprint 7 Confidence)
    ├── TrustScoreCard (Sprint 7 Verification)
    └── AIContributionCard (Sprint 7 AI vs Manual Analysis)
```

### 3. Component Responsibility Rule
- **`GitHubConnectionCard`**: ONLY OAuth status, Connect, Disconnect, Refresh. Never repository metadata.
- **`LinkedRepositoryBanner`**: ONLY Linked repository, Change repo, Sync, Webhook, Import status. Never analytics.
- **`RepositorySummarySidebar`**: ONLY Repository statistics (Commits, PRs, Issues, Reviews, Contributors). Never OAuth.

### 4. Data Ownership & Flow
`User → GitHub Connection → Projects → Repository Mapping (project_id) → Evidence → Analytics`
All telemetry and evidence items are strictly derived from the active `project_id`.

### 5. Backend Layering & Additive API Growth
`OAuth Service → Repository Service → Synchronization Service → Evidence Service → Analytics Service → AI Verification Service`
Do not mutate existing API contracts. Sprints add versioned resource endpoints (`/sync`, `/analytics`, `/tasks`, `/evidence`, `/trust-score`).

### 6. Regression-First Testing Policy
Every sprint preserves previous functionality (`pytest` suite, `npm run lint`, `npm run build`). No sprint is complete without 100% passing empirical verification across all previous sprint test suites.
