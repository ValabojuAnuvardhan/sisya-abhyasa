...
def main():
    ...
    try:
        ...
        if p2_title not in titles:
            ...
            db.add(m2); db.flush()
            db.add(Task(milestone_id=m2.id, title="Set up environment sensor script", description="Write local process inspector.", completion_criteria="Script detects running dev processes and logs metrics.", required_skills="Python\nGit", resources="Python OS Module\nProcess Monitoring Guide", position=1))
            db.commit()

        print("Seeded Demo 3 projects into Sprint 4B DB successfully.")
    finally:
        db.close()
