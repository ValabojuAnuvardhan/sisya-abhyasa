...
def team_project(project_id,user,db):
    p=db.scalar(select(Project).where(Project.id==project_id))
    if not p: raise HTTPException(404,'Project not found')
    if p.creator_id!=user.id:
        m=db.scalar(select(ProjectMember).where(ProjectMember.project_id==project_id,ProjectMember.user_id==user.id,ProjectMember.status=='active'))
...
def evidence(project_id:UUID,principal:AuthPrincipal=Depends(require_principal),db:Session=Depends(get_db)):
    user=user_for(principal,db); team_project(project_id,user,db); repo=db.scalar(select(ProjectRepository).where(ProjectRepository.project_id==project_id))
...
def link_pr(project_id:UUID,pr_id:UUID,payload:PullRequestTaskLink,principal:AuthPrincipal=Depends(require_principal),db:Session=Depends(get_db)):
    ...
    if project.creator_id!=user.id and pr.user_id!=user.id: raise HTTPException(403,'Contributors can only link their own attributed pull requests')
    if payload.task_id:
...
def github_identity_start(project_id:UUID,principal:AuthPrincipal=Depends(require_principal),db:Session=Depends(get_db)):
    ...
    user=user_for(principal,db); team_project(project_id,user,db)
