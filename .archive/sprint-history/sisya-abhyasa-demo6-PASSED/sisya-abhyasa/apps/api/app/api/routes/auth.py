...
def verify_email(payload:Verify,db:Session=Depends(get_db)):
    ...
    if not cred or not cred.verification_expires_at or cred.verification_expires_at < _now(): raise HTTPException(400,'Verification link is invalid or expired')
...
def login(payload:Login,response:Response,db:Session=Depends(get_db)):
    ...
    now=_now()
    if cred and cred.locked_until and cred.locked_until>now: raise HTTPException(429,'Too many failed attempts. Try again later.')
