...
def _safe_projection(user,db):
    ...
    member_proj_ids=select(ProjectMember.project_id).where(ProjectMember.user_id==user.id,ProjectMember.status=='active')
    projects=db.scalars(select(Project).where(or_(Project.creator_id==user.id,Project.id.in_(member_proj_ids))).order_by(Project.updated_at.desc())).all()
    out=[]
