...
from sqlalchemy import BigInteger, Boolean, DateTime, ForeignKey, Integer, String, Text, UniqueConstraint, func, Uuid as UUID, JSON as JSONB
...
class PrReview(Base):
    ...
    __table_args__=(UniqueConstraint('pull_request_id',name='uq_pr_reviews_pull_request'),)
    id: Mapped[uuid.UUID]=mapped_column(UUID(as_uuid=True),primary_key=True,default=uuid.uuid4)
    pull_request_id: Mapped[uuid.UUID]=mapped_column(UUID(as_uuid=True),ForeignKey('github_pull_requests.id',ondelete='CASCADE'),nullable=False,index=True)
    review_mode: Mapped[str]=mapped_column(String(30),nullable=False)
    source_scope: Mapped[str]=mapped_column(String(40),nullable=False)
    summary: Mapped[str]=mapped_column(Text,nullable=False)
    task_alignment: Mapped[str]=mapped_column(Text,nullable=False)
...
class SkillEvidence(Base):
    ...
    __table_args__=(UniqueConstraint('user_id','pull_request_id','skill_name',name='uq_skill_evidence_user_pr_skill'),)
    id: Mapped[uuid.UUID]=mapped_column(UUID(as_uuid=True),primary_key=True,default=uuid.uuid4)
